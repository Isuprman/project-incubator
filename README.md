...
Stage 4  实现循环        (工单)     → 每张: 设计门 → 测试 → 代码 → 三轴审查 → 完成后停下等你确认
```

## Execution rhythm（执行节奏）

- 每张工单内部**连续推进到完成**（测试 → 实现 → 重构 → 三轴审查 → 标记完成），不发送中间进度
- 只有三个闸门会停下来等你：开工前的测试 seam 确认、设计方向选择、**工单完成后的交付确认**
- 工单完成后汇报一次（交付物 + 测试结果 + 审查结论），你确认后才进入下一张；不会自动连跑

## Anti-drift（防上下文漂移）

长流程会稀释早期上下文（spec、设计简报、验收标准），agent 容易开始凭记忆干活。固定节点**强制重读文档**对抗漂移：

- 工单开工：重读工单文件（内含 Spec/Design 摘录）+ 验收标准
- 设计门（UI 工单）：重读 design-brief 对应页面全文
- 审查前：重读验收标准 + spec 摘录，逐条对照，偏离则 FAIL
- 每 2–3 张工单：重读全局 spec.md + CONTEXT.md（防跨工单漂移）

配套：Stage 3 拆工单时每张工单**内嵌 Spec excerpt**（UI 工单加 Design excerpt），
实现者只读工单文件即可开工，不依赖回忆。
